...
class SoftActorCritic(nn.Module):
    def __init__(
        ...
        if self.auto_tune_temperature:
            ...
            self.log_alpha = torch.tensor(np.log(self.temperature), requires_grad=True, device=ptu.device)
    ...
    def update_critic(
        ...
        with torch.no_grad():
            ...
            target_values = reward + self.discount * next_qs * (1.0 - done.float())
            # ENDTODO
    ...
    def actor_loss_reparametrize(self, obs: torch.Tensor):
        ...
        loss = -q_values.min(dim=0)[0].mean()
        # ENDTODO
